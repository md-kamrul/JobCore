...
import {
  ...
  uploadAvatar,
  uploadCV, getCVSignedUrl, deleteCV,
  getWorkExperience, saveWorkExperience,
...
const EditBar = ({ editing, onEdit, onSave, onCancel, saving }) => (
  <div style={{ display: "flex", gap: "8px" }}>
    {editing ? (
      <>
        <button onClick={onSave} disabled={saving} style={{ background: "#15803d", color: "white", border: "none", borderRadius: "8px", padding: "7px 16px", fontSize: "12px", fontWeight: 600, cursor: "pointer", opacity: saving ? 0.7 : 1 }}>
          {saving ? "Saving..." : "✓ Save"}
        </button>
...
const CVSection = ({ userId, cvMeta, onCvChange }) => {
  ...
  useEffect(() => {
    if (cvMeta?.cv_name && userId) {
      getCVSignedUrl(userId, cvMeta.cv_name).then(({ url }) => setCvUrl(url));
    } else {
      setCvUrl(null);
    }
  }, [cvMeta, userId]);
  ...
  const handleCvFile = async (file) => {
    ...
    setUploading(true);
    const { path, error } = await uploadCV(userId, file);
    ...
    await updateProfile(userId, newMeta);
    onCvChange(newMeta);
    setUploading(false);
  };

  const handleRemove = async () => {
    if (!userId || !cvMeta?.cv_name) return;
    await deleteCV(userId, cvMeta.cv_name);
    await updateProfile(userId, { cv_name: null, cv_size: null, cv_type: null, cv_uploaded_at: null, cv_note: null });
    onCvChange(null);
    setCvUrl(null);
  };
  ...
  const handleSaveNote = async () => {
    if (!userId) return;
    await updateProfile(userId, { cv_note: noteDraft });
    onCvChange({ ...cvMeta, cv_note: noteDraft });
    setEditingNote(false);
  };

  const isPdf = cvMeta?.cv_name && (cvMeta.cv_type === "application/pdf" || cvMeta.cv_name.endsWith(".pdf"));
  const uploadedAt = cvMeta?.cv_uploaded_at
    ? new Date(cvMeta.cv_uploaded_at).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })
    : "";
    ...
    <div style={{ marginTop: "20px" }}>
      ...
      {!cvMeta?.cv_name ? (
        <div
          ...
          style={{ border: `2px dashed ${dragging ? "#3b82f6" : "#334155"}`, borderRadius: "12px", padding: "28px 20px", textAlign: "center", cursor: uploading ? "wait" : "pointer", background: "#0f172a", transition: "all 0.2s" }}
        >
          <input
            ref={cvInputRef}
            type="file"
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "10px", pointerEvents: "none" }}>
            ...
            <div>
              <p style={{ color: "#e2e8f0", fontSize: "13px", fontWeight: 600, marginBottom: "3px" }}>
        ...
        <div>
          <div style={{ background: "#0f172a", borderRadius: "12px", border: "1px solid #1e3a5f", padding: "14px", display: "flex", gap: "14px", alignItems: "center", marginBottom: "12px" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ fontWeight: 600, fontSize: "13px", color: "#f1f5f9", margin: "0 0 3px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{cvMeta.cv_name}</p>
              <p style={{ fontSize: "11px", color: "#64748b", margin: 0 }}>{formatBytes(cvMeta.cv_size)} · Uploaded {uploadedAt}</p>
            </div>
            <div style={{ display: "flex", gap: "8px", flexShrink: 0 }}>
              {isPdf && cvUrl && (
                <a href={cvUrl} target="_blank" rel="noreferrer" style={{ display: "flex", alignItems: "center", gap: "5px", background: "#1e3a5f", color: "#60a5fa", border: "1px solid #2563eb44", borderRadius: "7px", padding: "5px 10px", fontSize: "11px", fontWeight: 600, textDecoration: "none" }}>
              ...
              {cvUrl && (
                <a href={cvUrl} download={cvMeta.cv_name} style={{ display: "flex", alignItems: "center", gap: "5px", background: "#052e16", color: "#4ade80", border: "1px solid #14532d44", borderRadius: "7px", padding: "5px 10px", fontSize: "11px", fontWeight: 600, textDecoration: "none" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
              ...
              {editingNote ? (
                <div style={{ display: "flex", gap: "6px" }}>
                  <button onClick={handleSaveNote} style={{ background: "#15803d", color: "white", border: "none", borderRadius: "6px", padding: "4px 10px", fontSize: "11px", cursor: "pointer" }}>✓ Save</button>
                ...
                <button onClick={() => { setNoteDraft(cvMeta?.cv_note || ""); setEditingNote(true); }} style={{ background: "transparent", color: "#60a5fa", border: "none", fontSize: "11px", cursor: "pointer", padding: 0 }}>
                  {cvMeta?.cv_note ? "Edit note" : "+ Add note"}
                </button>
              )}
            </div>
            {editingNote
              ? <TA rows={2} value={noteDraft} placeholder="e.g. Latest version — updated April 2026" onChange={(e) => setNoteDraft(e.target.value)} />
              : <p style={{ color: cvMeta?.cv_note ? "#cbd5e1" : "#334155", fontSize: "12px", margin: 0, fontStyle: cvMeta?.cv_note ? "normal" : "italic" }}>{cvMeta?.cv_note || "No note added."}</p>
            }
          </div>
        </div>
      )}
...
const ProfileSection = () => {
  ...
  useEffect(() => {
    if (!user?.id) return;
    const load = async () => {
      const { data: p } = await getProfile(user.id);
      if (p) {
        setProfile(p);
        setProfileDraft(p);
        setContact({ email: p.email || "", phone: p.phone || "", website: p.website || "", linkedin: p.linkedin || "" });
        setContactDraft({ email: p.email || "", phone: p.phone || "", website: p.website || "", linkedin: p.linkedin || "" });
        if (p.cv_name) setCvMeta({ cv_name: p.cv_name, cv_size: p.cv_size, cv_type: p.cv_type, cv_uploaded_at: p.cv_uploaded_at, cv_note: p.cv_note });
      }
      const { data: w } = await getWorkExperience(user.id);
      if (w) { setWork(w); setWorkDraft(w.map(x => ({ ...x }))); }
      const { data: e } = await getEducation(user.id);
      if (e) { setEdu(e); setEduDraft(e.map(x => ({ ...x }))); }
  ...
  const handleSaveProfile = async () => {
    ...
    await updateProfile(user.id, {
      ...
      desired_role: profileDraft.desired_role,
    });
  ...
  const handleSaveWork = async () => {
    if (!user?.id) return;
    setSavingWork(true);
    await saveWorkExperience(user.id, workDraft);
    setWork([...workDraft]);
    setSavingWork(false);
  ...
  const handleSaveEdu = async () => {
    if (!user?.id) return;
    setSavingEdu(true);
    await saveEducation(user.id, eduDraft);
    setEdu([...eduDraft]);
    setSavingEdu(false);
    setEditingEdu(false);
  };

  const updateWorkDraft = (id, field, val) => setWorkDraft(prev => prev.map(w => w.id === id ? { ...w, [field]: val } : w));
  const updateEduDraft = (id, field, val) => setEduDraft(prev => prev.map(e => e.id === id ? { ...e, [field]: val } : e));
  const addWork = () => setWorkDraft(prev => [...prev, { id: `new-${Date.now()}`, role: "", company: "", period: "", description: "", color: COLORS[prev.length % COLORS.length] }]);
  const removeWork = (id) => setWorkDraft(prev => prev.filter(w => w.id !== id));
  const addEdu = () => setEduDraft(prev => [...prev, { id: `new-${Date.now()}`, degree: "", school: "", period: "", gpa: "", color: COLORS[(prev.length + 2) % COLORS.length] }]);
  ...
  const tabs = [{ key: "overview", label: "Overview" }, { key: "work", label: "Work & Education" }, { key: "contact", label: "Contact & Info" }];

  if (pageLoading) {
    return (
      <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#0a0f1e 0%,#0d1829 50%,#091424 100%)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <p style={{ color: "#60a5fa", fontSize: "14px" }}>Loading profile...</p>
      </div>
    );
  }
  ...
  if (!user?.id) {
    return (
      <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#0a0f1e 0%,#0d1829 50%,#091424 100%)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <p style={{ color: "#ef4444", fontSize: "14px" }}>Please log in to view your profile</p>
      </div>
    );
  }
      ...
      <div style={{ maxWidth: "1080px", margin: "0 auto" }}>
        ...
        <div style={{ background: "linear-gradient(135deg,#1e293b,#162032)", borderRadius: "20px", padding: "28px", marginBottom: "20px", border: "1px solid #1e3a5f", display: "flex", alignItems: "center", gap: "22px", flexWrap: "wrap", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ flex: 1, minWidth: "200px" }}>
            {editingProfile ? (
              <>
                <IN value={profileDraft?.full_name || ""} onChange={e => setProfileDraft({ ...profileDraft, full_name: e.target.value })} style={{ fontSize: "20px", fontWeight: 700, marginBottom: "8px" }} />
                <TA value={profileDraft?.bio || ""} onChange={e => setProfileDraft({ ...profileDraft, bio: e.target.value })} rows={2} />
              </>
            ) : (
              <>
                <h1 style={{ fontSize: "24px", fontWeight: 700, color: "#f1f5f9", margin: "0 0 3px" }}>{profile?.full_name || "Your Name"}</h1>
                <p style={{ color: "#60a5fa", fontSize: "13px", margin: "0 0 8px" }}>{profile?.desired_role || "Your Role"}</p>
                <p style={{ color: "#94a3b8", fontSize: "13px", lineHeight: 1.6, margin: 0 }}>{profile?.bio || "Add a bio to tell employers about yourself."}</p>
              </>
            )}
          </div>
          <EditBar editing={editingProfile} saving={savingProfile}
        ...
        <div style={{ flex: 1, minWidth: "280px" }}>
          ...
          {activeTab === "overview" && (
            <div style={cardStyle}>
              ...
              <Field label="Full Name" value={editingProfile ? profileDraft?.full_name : profile?.full_name} editing={editingProfile} onChange={v => setProfileDraft({ ...profileDraft, full_name: v })} />
              ...
              <Field label="Desired Role" value={editingProfile ? profileDraft?.desired_role : profile?.desired_role} editing={editingProfile} onChange={v => setProfileDraft({ ...profileDraft, desired_role: v })} />
              ...
              <div style={{ borderTop: "1px solid #0f172a", marginTop: "6px", paddingTop: "20px" }}>
                <CVSection userId={user.id} cvMeta={cvMeta} onCvChange={(meta) => setCvMeta(meta)} />
              </div>
            </div>
          )}
              ...
              <div style={cardStyle}>
                <div style={sectionHead}>
                  ...
                  <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                    ...
                    <EditBar editing={editingWork} saving={savingWork}
              ...
              <div style={cardStyle}>
                <div style={sectionHead}>
                  ...
                  <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                    ...
                    <EditBar editing={editingEdu} saving={savingEdu}
          ...
          {activeTab === "contact" && (
            <div style={cardStyle}>
              <div style={sectionHead}>
                ...
                <EditBar editing={editingContact} saving={savingContact}
